-- OBS Custom Bounce v1.6 by Mike F. (A.K.A. Sorcerer's Sundries)
...
function script_description()
	return	'Bounce multiple sources (by name) around their scene\n\n' ..
			'Key features include Settings to...\n' ..
			' - Set a staggered startup delay for multiple objects\n' ..
			' - Apply physics to the bouncing with multiple physics settings\n'..
			' - Choose movement angle ranges and initial direction\n'..
			' - Put a random spin on it (NON-PHYSICS is randomized, PHYSICS is speed based)\n' ..
			' - Use offsets to limit the bounce region\n' ..
			' - Set 3 different auto-off triggers:\n' ..
			'      - A general-use timer \n' ..
			'      - A bounce-count limit (NON-PHYSICS only)\n' ..
			'      - A timer that starts when all objects are at rest (PHYSICS only)\n' ..
			'           - Including an auto-restart option after Physics auto-off triggers\n\n' ..
			'Code v1.6   by Mike F.'
end
...
function script_properties()
	...
	obs.obs_properties_add_int(props, 'SceneMinXOffset', 'Bounce area offset (Pixels): Left', -1000,4000,50)
	obs.obs_properties_add_int(props, 'SceneMaxXOffset', 'Bounce area offset (Pixels): Right', -1000,4000,50)
	obs.obs_properties_add_int(props, 'SceneMaxYOffset', 'Bounce area offset (Pixels): Top', -1000,4000,50)
	obs.obs_properties_add_int(props, 'SceneMinYOffset', 'Bounce area offset (Pixels): Bottom', -1000,4000,50)
	...
	obs.obs_properties_add_int(props, 'AutoOffBounceLimit', 'NON-PHYSICS: Auto-off bounce amount', 1,1000,10)
...
function script_load(settings)
	hotkey_id = obs.obs_hotkey_register_frontend('toggle_bounce', 'MF Toggle Bounce', toggle)
